...
class manipulation():
    def __init__(self):
        ...
        btninsert=Button(frametop,text="INSERT", command=self.insertvalues).pack(padx=10, pady=10)
        btnupdate=Button(frametop,text="UPDATE", command=self.update).pack(padx=10, pady=10)
        tndelete=Button(frametop,text="DELETE",command=self.deletevalues).pack(padx=10, pady=10)

    def MyDBConnection(manipulation):
    ...
    #statement="update student_marks set value1=(%s) where sno=(%s);"
    #valuepass=(val1,val2)
...
clickme=Button(app,text="Insert",bg="red",command=insertvalues)
